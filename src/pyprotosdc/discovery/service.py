...
class Service:
    ...
    def __repr__(self) -> str:
        scopes_str = 'None' if self.scopes is None else ', '.join([str(x) for x in self.scopes])
        return f'Service epr={self.epr}, Xaddr={self._x_addrs} ' \
               f'scopes={scopes_str}'

    def __str__(self) -> str:
        scopes_str = 'None' if self.scopes is None else ', '.join([str(x) for x in self.scopes])
        return f'Service epr={self.epr}, \n' \
