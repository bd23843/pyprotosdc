...
if TYPE_CHECKING:
    ...
    from .discoveryimpl import GDiscovery
...
class OutgoingMessage:
    ...
    created_message: discovery_messages_pb2.DiscoveryMessage
    addr: str
    port: int
...
class _NetworkingThreadBase(ABC):
    ...
    def __init__(self,
                 my_ip_address: str,
                 gdiscovery: GDiscovery,
                 logger: Logger,
                 multicast_port: int):
        self._my_ip_address = my_ip_address
        self._gdiscovery = gdiscovery
        self._logger = logger
    ...
    def add_unicast_message(self,
                            ...
                            addr: str,
                            port: int):
        out_msg = OutgoingMessage(p_msg, addr, port, _MessageType.UNICAST)
        self._logger.debug('add_unicast_message: adding message Id %s',
                           p_msg.addressing.message_id)
        self._repeated_enqueue_msg(out_msg, unicast_repeat_params)

    def add_multicast_message(self,
                              p_msg: discovery_messages_pb2.DiscoveryMessage,
                              addr: str,
                              port: int):
        out_msg = OutgoingMessage(p_msg, addr, port, _MessageType.MULTICAST)
        self._logger.debug('add_multicast_message %s: adding message Id %s',
                           p_msg.addressing.action,
                           p_msg.addressing.message_id)
        self._repeated_enqueue_msg(out_msg, multicast_repeat_params)
