...
from pyprotosdc.consumer.operations import OperationResult
from pyprotosdc.mapping.basic_mappers import enum_attr_from_p, enum_from_p
...
def connect_client(my_service) -> GSdcConsumer:
    if ca_folder:
        ...
        ssl_contexts = None
    client = GSdcConsumer.from_service(my_service,
                                       ssl_context_container=ssl_contexts)
    client.subscribe_all()
    return client


def init_mdib(client) -> GClientMdibContainer:
    # The Reference Provider answers to GetMdib
    mdib = GClientMdibContainer(client)
    mdib.init_mdib()
    return mdib
...
def run_ref_test():
    ...
    while my_service is None:
        ...
        for s in services:
            if s.epr.endswith(search_epr):
                my_service = s
                print(f'found service {s.epr} at {s.x_addrs}')
                break
    ...
    context_service = client.get_service
    if context_service is None:
        results.append(f'{step} => failed {info}')
    else:
        try:
            response_data = context_service.get_context_states()
        ...
        loc_states = [ s for s in response_data.states if s.NODETYPE == pm_qnames.LocationContextState]
    ...
    def on_metric_updates(metrics_by_handle):
        # print('on_metric_updates', metrics_by_handle)
        for k, v in metrics_by_handle.items():
            print(f'on_metric_updates: State {v.NODETYPE.localname} {v.DescriptorHandle}')
    ...
    def on_alert_updates(alerts_by_handle):
        for k, v in alerts_by_handle.items():
            print(f'on_alert_updates:State {v.NODETYPE.localname} {v.DescriptorHandle}')
    ...
    for report in description_updates:
        for report_part in report.description.report_part:
            ...
            if modification_type == msg_types.DescriptionModificationType.UPDATE:
                for descriptor in report_part.p_descriptor:
                    which = descriptor.WhichOneof(descriptor.DESCRIPTOR.oneofs[0].name)
                    if which == 'alert_condition_descriptor':
                        found = True
    ...
    for report in description_updates:
        for report_part in report.description.report_part:
            ...
            if modification_type == msg_types.DescriptionModificationType.UPDATE:
                for descriptor in report_part.p_descriptor:
                    which = descriptor.WhichOneof(descriptor.DESCRIPTOR.oneofs[0].name)
                    if which == 'numeric_metric_descriptor':
                        found = True
    log_result(found, results, step, info)
    ...
    for report in description_updates:
        for report_part in report.description.report_part:
            modification_type = enum_attr_from_p(report_part,
                                                 'ModificationType',
                                                 msg_types.DescriptionModificationType)
            if modification_type == msg_types.DescriptionModificationType.CREATE:
                for descriptor in report_part.p_descriptor:
                    which = descriptor.WhichOneof(descriptor.DESCRIPTOR.oneofs[0].name)
                    if which == 'vmd_descriptor':
                        add_found = True
            if modification_type == msg_types.DescriptionModificationType.DELETE:
                for descriptor in report_part.p_descriptor:
                    which = descriptor.WhichOneof(descriptor.DESCRIPTOR.oneofs[0].name)
                    if which == 'vmd_descriptor':
                        rm_found = True
    ...
    if len(patient_context_descriptors) == 0:
        ...
        try:
            for i, p in enumerate(patient_context_descriptors):
                pat = mdib.xtra.mk_proposed_state(p.Handle)
                pat.Handle = p.Handle  # new patient
                ...
                fut = client.set_service.set_context_state(set_context_state_handle, [pat])
                op_result: OperationResult = fut.result()
                if op_result.InvocationInfo.InvocationState != msg_types.InvocationState.FINISHED:
                    log_result(False, op_result, step, info, extra_info=op_result.InvocationInfo.InvocationErrorMessage)
    ...
    print(step, info)
    try:
        operations = mdib.descriptions.NODETYPE.get(pm_qnames.SetValueOperationDescriptor, [])
    ...
    print(step, info)
    try:
        operations = mdib.descriptions.NODETYPE.get(pm_qnames.SetStringOperationDescriptor, [])
        ...
        if len(my_ops) != 1:
            ...
            operation = my_ops[0]
            future_object = client.set_service.set_string(operation.Handle, 'STANDBY')
            operation_result = future_object.result(timeout=4)
            if len(operation_result.report_parts) == 0:
                log_result(False, results, step, info, 'no notification')
                ...
                inv_states = [enum_from_p(p.invocation_info, 'invocation_state', msg_types.InvocationState) for p in operation_result.report_parts]
    ...
    print(step, info)
    try:
        operations = mdib.descriptions.NODETYPE.get(pm_qnames.SetMetricStateOperationDescriptor, [])
        ...
        if len(my_ops) != 1:
            ...
            operation = my_ops[0]
            proposed_metric_state1 = mdib.xtra.mk_proposed_state("numeric_metric_0.channel_0.vmd_1.mds_0")
            proposed_metric_state2 = mdib.xtra.mk_proposed_state("numeric_metric_1.channel_0.vmd_1.mds_0")
            ...
            future_object = client.set_service.set_metric_state(operation.Handle, [proposed_metric_state1, proposed_metric_state2])
            operation_result = future_object.result(timeout=4)
            if len(operation_result.report_parts) == 0:
                log_result(False, results, step, info, 'no notification')
            elif len(operation_result.report_parts) > 1:
